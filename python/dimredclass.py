...
ldims=[1,2,3,5,9,16,27,47,81,128]
nDim = len(ldims)
...
def dimred(iCategory,catname,nOutputDim):
    ...
    dataOri = np.loadtxt(catfilename,dtype=np.float32)       
    dataOri = dataOri[:,:-2]
...
def main():
    ...
    nSamplesPerCat = int(np.round(nSamples/nCategory))
    for iLDim,ldim in enumerate(ldims):
        ...
        for iCategory,catname in enumerate(catList):
            ...
            for imgid in ImgId:
                ...
                if imgcount==0:
                    dataout = np.hstack((hist.T,imgid))
                else:
                    dataout = np.vstack((dataout,np.hstack((hist.T,imgid))))
                imgcount+=1
        ...
        for iCategory,catname in enumerate(catList):
            ...
            y2 = np.where(y!=posLabel,negLabel,y)
            ...
            for i, (train, test) in enumerate(cv):
                ...
                roc_auc = auc(fpr, tpr)
                precision, recall, thresholds = precision_recall_curve(y2[test], probas_[:,1]) #@UnusedVariable
